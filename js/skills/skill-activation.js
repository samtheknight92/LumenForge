  ...
  if (Array.isArray(skill.activationEffects) && skill.activationEffects.length) {
    ...
    return skill.activationEffects
      ...
      .map(item => ({
        ...
        applyTo: item.applyTo || skill.defaultApplyTo || 'self',
        source: 'data'
      }))
  }
...
export function getPinnedActionBarSkills(character) {
  if (!character?.pinnedSkillIds?.length) return []
  return character.pinnedSkillIds
    .map(id => getSkill(id))
...
export function getActionBarSkills(character) {
  ...
  const pinned = new Set(character.pinnedSkillIds || [])
  return character.skills
    .map(id => getSkill(id))
    .filter(skill => skill && isActionBarSkill(skill) && !pinned.has(skill.id))
