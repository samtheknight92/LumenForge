...
import { HIDDEN_SKILL_CATEGORIES, SKILL_SUBCATEGORY_LABELS, TIER_MIN_LEVEL, CAPSTONE_TIER_MIN_LEVEL } from '../core/constants.js'
import { cache, flattenSkills, getSkill, displayCategory, getRace } from '../core/cache.js'
import {
  listHomebrewSkills,
  listHomebrewRaces,
...
import { activeCharacter, state } from '../core/state.js'
...
function isCapstoneSkillUnlocked(character, skill) {
  ...
  if (computeSkillLevel(character).skillLevel < minLevelToLearnSkill(skill)) return false
...
function evaluatePrerequisiteReq(character, req) {
  ...
  if (req.type === 'LEVEL') {
    return computeSkillLevel(character).skillLevel >= Number(req.level || 0)
  }
  if (req.anyOfSkills?.length) {
...
export function canLearnSkill(character, skill) {
  ...
  if (computeSkillLevel(character).skillLevel < needLevel) {
    return {
      ok: false,
      reason: isHomebrewSkill(skill)
        ? `Requires Skill Level ${needLevel}`
        : `Requires Skill Level ${needLevel} (Tier ${skill.tier || 1})`
    }
  }
  if (!hasPrerequisites(character, skill)) {
    const req = skill?.prerequisites
    if (req?.type === 'LEVEL') return { ok: false, reason: `Requires Skill Level ${req.level}` }
    if (isHomebrewSkill(skill) && skill.lockSkills?.length) {
    ...
    return { ok: false, reason: label !== `Skill Level ${minLevelToLearnSkill(skill)}+` ? `Requires ${label}` : 'Missing prerequisite' }
  }
