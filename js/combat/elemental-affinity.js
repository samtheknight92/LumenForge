...
import { getEffect, normalizeEffectId } from '../character/character.js'
...
/** Map GM builder / template tags to canonical element ids (fire, ice, light, …). */
const BUILDER_ELEMENT_ALIASES = {
  holy: 'light',
  sacred: 'light',
  necrotic: 'darkness',
  dark: 'darkness',
...
export function normalizeBuilderElementId(name) {
  const clean = String(name || '').trim().toLowerCase()
  if (!clean) return ''
  const aliased = BUILDER_ELEMENT_ALIASES[clean] || clean
  return normalizeElementId(aliased)
}

function normalizeBuilderTagList(tags) {
  return [...new Set((tags || []).map(normalizeBuilderElementId).filter(Boolean))]
}
...
export function collectElementalAffinitySources(character) {
  ...
  const gb = character.gmBuilder
  if (gb) {
    const source = character.name ? `Type: ${character.name}` : 'GM Builder type'
    for (const tag of gb.resistances || []) {
      const element = normalizeBuilderElementId(tag)
      if (element) pushEntry(entries, { element, kind: 'resistance', tier: 50, source })
    }
    for (const tag of gb.weaknesses || []) {
      const element = normalizeBuilderElementId(tag)
      if (element) pushEntry(entries, { element, kind: 'weakness', tier: 200, source })
    }
    for (const tag of gb.immunities || []) {
      const element = normalizeBuilderElementId(tag)
      if (element) pushEntry(entries, { element, kind: 'immunity', tier: 0, source })
    }
  }
...
/** Resist / weak / immune labels for GM preview — matches the character sheet affinity grid. */
export function elementalAffinityTraitLabels(character) {
  const profile = computeElementalAffinity(character)
  const resists = []
  const weaknesses = []
  const immunities = []
  for (const row of Object.values(profile.elements || {})) {
    if (!isElementalAffinityRowVisible(row)) continue
    const label = row.id || row.name?.toLowerCase()
    if (!label) continue
    if (row.kind === 'resistance') resists.push(label)
    else if (row.kind === 'weakness') weaknesses.push(label)
    else if (row.kind === 'immunity') immunities.push(label)
  }
  const gb = character?.gmBuilder || {}
  for (const tag of gb.immunities || []) {
    if (!normalizeBuilderElementId(tag) && tag) immunities.push(String(tag).toLowerCase())
  }
  return { resists, weaknesses, immunities }
}
