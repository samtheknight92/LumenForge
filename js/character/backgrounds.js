import { DEFAULT_STARTING_GIL, DEFAULT_STARTING_LUMENS } from '../core/constants.js'
import { getItem } from '../core/cache.js'
import { addItemToInventory } from '../items/items.js'
import { formatCurrency } from '../ui/format.js'
import { getHomebrewBackground, listHomebrewBackgrounds } from '../homebrew/homebrew.js'
...
export function getBackground(id) {
  const homebrew = getHomebrewBackground(id)
  if (homebrew) return homebrew
  return BACKGROUNDS[id] || BACKGROUNDS[DEFAULT_BACKGROUND]
}

export function backgroundOptions() {
  const official = Object.values(BACKGROUNDS)
  const custom = listHomebrewBackgrounds({ playerFacing: true })
...
export function applyBackgroundToCharacter(character, backgroundId) {
  ...
  const hardLine = bg.hardMode ? '\n\nHard mode background — GM reference.' : ''
  character.notes = character.notes ? `${character.notes}\n\n${noteLine}${tableLine}${hardLine}` : `${noteLine}${tableLine}${hardLine}`
}
