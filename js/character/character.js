import { DEFAULT_STATS, STAT_RULES, DEFAULT_STARTING_GIL, DEFAULT_STARTING_LUMENS } from '../core/constants.js'
import { cache, getRace, getSkill, getItem } from '../core/cache.js'
import { listHomebrewSkills } from '../homebrew/homebrew.js'
import { equipmentSkillStatModifiers } from '../skills/skill-effects.js'
import { armourSkillStatModifiers, conditionalSkillStatModifiers, conditionalSkillStatLabel, evaluateSkillStatCondition } from '../skills/career-effects.js'
...
import { characterWieldsWeaponKind, reconcileOffhandEquip } from '../items/equipment.js'
import { getSkillsData } from '../core/data.js'
import { clamp, deepClone, uid, titleCase } from '../core/utils.js'
import { HUMAN_RACE_SKILL } from '../skills/skills.js'
import { normalizeGil } from '../ui/format.js'
...
export function normalizeCharacter(character) {
  ...
  merged.notePages = migrateNotePages(character)
  merged.quests = migrateQuests(character)
  merged.pinnedSkillIds = Array.isArray(character?.pinnedSkillIds)
    ? character.pinnedSkillIds.filter(id => merged.skills.includes(id))
    : []
  merged.starredSkillIds = Array.isArray(character?.starredSkillIds)
    ? character.starredSkillIds.filter(id => merged.skills.includes(id))
    : []
  merged.starredRecipeIds = Array.isArray(character?.starredRecipeIds)
    ? character.starredRecipeIds.map(String)
    : []
...
export function normalizeInventoryEntry(entry) {
  ...
  if (typeof entry === 'string') return { uid: uid('item'), itemId: entry, qty: 1, acquiredAt: new Date().toISOString() }
  ...
  return {
    ...
    playerNotes: String(entry.playerNotes || '').trim().slice(0, 2000) || undefined,
    acquiredAt: String(entry.acquiredAt || new Date().toISOString())
  }
}

function migrateNotePages(character) {
  if (Array.isArray(character?.notePages) && character.notePages.length) {
    return character.notePages.map(row => ({
      id: String(row.id || uid('note')),
      title: String(row.title || 'Notes').trim().slice(0, 80) || 'Notes',
      body: String(row.body || '')
    }))
  }
  const legacy = String(character?.notes || '').trim()
  if (legacy) {
    return [{ id: uid('note'), title: 'General Notes', body: legacy }]
  }
  return [{ id: uid('note'), title: 'General Notes', body: '' }]
}

function migrateQuests(character) {
  if (!Array.isArray(character?.quests)) return []
  return character.quests.map(row => ({
    id: String(row.id || uid('quest')),
    name: String(row.name || '').trim().slice(0, 120),
    giver: String(row.giver || '').trim().slice(0, 80),
    reward: String(row.reward || '').trim().slice(0, 200),
...
export function normalizeStatusEffect(entry) {
  ...
  if (Number.isFinite(Number(migrated.combatRoll))) {
    const roll = Math.max(1, Math.min(6, Math.floor(Number(migrated.combatRoll))))
    normalized.combatRoll = roll
  }
  if (migrated.finiteDuration) normalized.finiteDuration = true
  return normalized
}
...
export function computeStats(character) {
  ...
  for (const status of character?.weatherEffects || []) {
    const effect = getEffect(status.effectId)
    for (const [stat, value] of Object.entries(statusStatModifiers(status, effect))) {
      stats[stat] = (stats[stat] || 0) + Number(value || 0)
    }
