...
import { getEffect, normalizeEffectId, normalizeStatusEffect, invalidateCharacterCache, computeStats } from '../character/character.js'
import { characterWieldsWeaponKind } from '../items/equipment.js'
import { resolveSkillGearEffects } from '../skills/skill-effects.js'
import {
  entryEnchantments,
...
import { activeMaxStatRewards, maxStatRewardSourceLabel } from '../character/max-stat-rewards.js'
...
export function isWeatherEffect(effect) {
  return String(effect?.type || '').toLowerCase() === 'weather'
}

export function groupedWeatherEffects() {
  return effectList().filter(effect => isWeatherEffect(effect))
}
...
export function effectTooltip(effectId, source = '', status = null) {
  ...
  if (gameplay.length) {
    lines.push('', 'Gameplay:')
    gameplay.forEach(line => lines.push(`• ${line}`))
  }
...
const RACE_TRAIT_EFFECT_RULES = [
  ...
  [/immune\s+to\s+[^.]*\bfire(?:\s+damage)?/i, 'fire_immunity'],
  [/immune\s+to\s+[^.]*\bcharm\b/i, 'charm_immunity'],
  [/immune\s+to\s+[^.]*\bfear\b/i, 'fear_immunity'],
...
export { resolveSkillEffects, resolveSkillGearEffects } from '../skills/skill-effects.js'
...
export function tickWeatherEffects(character) {
  const statuses = Array.isArray(character.weatherEffects) ? character.weatherEffects : []
  ...
  for (const status of statuses) {
    ...
    if (Number(status.duration) === 0 && (status.finiteDuration || effect.duration !== 0)) expired += 1
    else kept.push(status)
  }
  character.weatherEffects = kept
  invalidateCharacterCache(character)
  return { summary: expired ? `${expired} weather effect${expired === 1 ? '' : 's'} cleared` : '' }
}
