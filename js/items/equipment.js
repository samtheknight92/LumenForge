...
export function isTwoHandedWeapon(item) {
  ...
  const hands = String(item.hands ?? item.handedness ?? '').toLowerCase()
  if (hands === 'two' || hands === 'two_handed' || hands === '2' || hands === 2 || item.twoHanded === true) return true
  if (hands === 'one' || hands === 'one_handed' || hands === '1' || hands === 1 || item.twoHanded === false) return false
