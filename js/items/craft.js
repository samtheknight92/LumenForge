...
import { listHomebrewRecipes } from '../homebrew/homebrew.js'

export function listCraftRecipes() {
  const data = getGameData()['profession-items'] || {}
  const official = Object.entries(data).flatMap(([profession, items]) =>
    Object.values(items || {}).map(item => ({ ...item, profession }))
  )
  const homebrew = listHomebrewRecipes({ playerFacing: true }).map(recipe => ({
    ...recipe,
    id: recipe.id,
  ...
  return [...official, ...homebrew]
    .filter(recipe => !recipe.outputItemId || getItem(recipe.outputItemId))
...
export function filterCraftRecipes(character, { search = '', profession = 'all', learnedOnly = true, starredOnly = false } = {}) {
  const query = String(search || '').trim().toLowerCase()
  const starred = new Set(character?.starredRecipeIds || [])
  return listCraftRecipes().filter(recipe => {
    ...
    if (starredOnly && !starred.has(recipe.id)) return false
