import { uid, titleCase } from '../core/utils.js'
import { ITEMS_PER_PAGE, SHOP_MIN_LEVEL_BY_RARITY } from '../core/constants.js'
import { cache, getItem, itemSources, getItemSearchText, rarityRank } from '../core/cache.js'
import { activeCharacter, state } from '../core/state.js'
import { normalizeGil, itemPriceGil, damageAverage } from '../ui/format.js'
...
export function itemBlocksRemoveWhenLocked(entry) {
...
function matchesCatalogFilters(item, character, skip = {}) {
  ...
  if (!skip.starred && state.itemStarredOnly && !(state.starredCatalogItemIds || []).includes(item.id)) return false
  return true
}
...
export function isShopPurchaseItem(item) {
  ...
  if (item?.source === 'homebrew') {
    if (item.archived || item.approvalStatus === 'hidden' || item.approvalStatus === 'draft') return false
    if (item.sellable === false) return false
    return item.listInShop && itemPriceGil(item) > 0
  }
  return false
}
...
export function isShopItemLevelUnlocked(character, item) {
  ...
  return computeSkillLevel(character).skillLevel >= shopMinLevelForItem(item)
}
