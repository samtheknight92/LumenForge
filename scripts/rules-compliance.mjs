...
const UNIDENTIFIED_WEAPON_IDS = new Set([
  'unknown_sword', 'unknown_axe', 'unknown_dagger', 'unknown_staff', 'unknown_bow',
  'unknown_hammer', 'unknown_polearm', 'unknown_ring', 'unknown_amulet', 'unknown_cloak',
  'unknown_armour', 'unknown_shield', 'unknown_vial', 'unknown_scroll', 'unknown_item'
])
const noDmg = weapons.filter(i => !i.damage && !i.unidentifiedItem && !UNIDENTIFIED_WEAPON_IDS.has(i.id))
if (noDmg.length) {
...
const RETIRED_CAREERS = ['merchant', 'farmer', 'engineer', 'scholar', 'archaeologist', 'beast_handler', 'scout']
for (const key of RETIRED_CAREERS) {
  if (careers[key]) add('ERROR', 'careers', `${key} career still in careers-skills-data.js`)
}
...
// ── Retired profession keys on craft recipes ──
const RETIRED_PROFESSIONS = ['merchant', 'farmer', 'engineer', 'scholar', 'archaeologist', 'beast_handler', 'scout', 'herbalism', 'smithing', 'cooking', 'archaeology', 'enchanting', 'alchemy']
const badProf = walkItems(prof).filter(i => RETIRED_PROFESSIONS.includes(String(i.profession || '').toLowerCase()))
if (badProf.length) {
