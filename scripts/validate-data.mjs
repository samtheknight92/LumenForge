...
for (const { skill, path: skillPath } of skillRows) {
  ...
  const prereqIds = [
    ...(skill.prerequisites?.skills || []),
    ...(skill.alternativePrerequisite?.skills || []),
    ...(skill.prerequisites?.anyOfSkills || []),
    ...(skill.alternativePrerequisite?.anyOfSkills || [])
  ]
  ...
  const capstone = CAPSTONE_ACTIVATION_EFFECTS[skill.id]
  const expectedActivation = capstone?.length
    ? capstone
    : resolveActivationEffectsForSkill(skill, effects, {
...
const requiredManualEffects = collectRequiredManualEffectIds(effects)
const manualEffectAllowlist = new Set(loadPlayerManualEffectIdsFromSource())
for (const effectId of requiredManualEffects.keys()) {
  if (!manualEffectAllowlist.has(effectId)) {
    errors.push(`Add Effect picker missing trackable status "${effectId}" (used by skills/consumables)`)
  }
}
for (const effectId of manualEffectAllowlist) {
  if (!effects[effectId]) errors.push(`Add Effect picker lists unknown effect "${effectId}"`)
}
