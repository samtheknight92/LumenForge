...
const CP_WEIGHTS = { hp: 0.2, stamina: 0.2, strength: 0.4, magicPower: 0.4, accuracy: 0.4, speed: 0.4, physicalDefence: 0.4, magicalDefence: 0.4 }

function starterSkillLevel(entry) {
  const skillLevels = (entry.skills || []).reduce((sum, id) => sum + tierLevel(skillById.get(id)?.tier), 0)
  return Math.floor(skillLevels) + 1
}

function starterCombatPower(entry) {
  const stats = { ...DEFAULT_STATS, ...(entry.stats || {}) }
  let total = 0
  for (const [key, weight] of Object.entries(CP_WEIGHTS)) {
    const delta = Math.max(0, Number(stats[key] ?? DEFAULT_STATS[key]) - Number(DEFAULT_STATS[key]))
    total += delta * weight
  }
...
console.log('## Lumens per progression point (economy)')
console.log('  Skill Level — tier cost floors:', Object.entries(TIER_LUMEN_COST).map(([t, c]) => `T${t}=${c}L`).join(' · '))
console.log('  Combat Power — stat purchases: HP/STA 3–4L per +0.2 · other stats 8–12L per +0.4')
console.log('  Milestone gates:', Object.entries(TIER_MIN_LEVEL).map(([t, lv]) => `T${t}→Skill Lv${lv}`).join(' · '))
console.log('')
...
console.log('\n## Quick-Start player sheets (tier ≤2 at creation · T3 unlocks Skill Lv8)')
for (const entry of level5Starters) {
  ...
  const skillLevel = starterSkillLevel(entry)
  const combatPower = starterCombatPower(entry)
  console.log(`  ${entry.name} (${entry.premadeId})`)
  console.log(`    Skill Level ${skillLevel} · Combat Power ${combatPower} · legacy combined level ${spend.level}`)
  console.log(`    ${(entry.skills || []).length} skills (max tier ${maxTier}) · HP ${entry.stats?.hp ?? DEFAULT_STATS.hp}`)
