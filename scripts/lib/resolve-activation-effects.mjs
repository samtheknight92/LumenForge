...
function inferApplyTo(desc, effect, options = {}) {
  if (options.applyTo) return options.applyTo
  if (options.fromChance) return 'target'
  if (!options.fromApply || !effect) return undefined
  const text = String(desc || '')
  if (/to yourself|on yourself|yourself\b/i.test(text)) return 'self'
  if (isBadEffect(effect)) return 'target'
  return 'self'
}

function selfApplyEffectType(effect) {
  const type = String(effect?.type || '').toLowerCase()
  return type.includes('protection') || type.includes('buff') || type.includes('recovery')
    || type.includes('heal') || type.includes('enhancement') || type.includes('movement')
}
...
function parseApplyPhrases(desc, effects, skill) {
  ...
  function addPhrase(phrase, parenText, options = {}) {
    ...
    if (options.applyTo) row.applyTo = options.applyTo
    else {
      const inferred = inferApplyTo(text, effect, options)
      if (inferred) row.applyTo = inferred
    }
    if (options.fromChance) {
      const pctInPhrase = String(parenText || text).match(/(\d+)%\s+chance/i)
      row.chance = pctInPhrase
        ? Number(pctInPhrase[1]) / 100
        : (applyChanceFromDesc(text, skill?.tier) ?? TIER_APPLY_CHANCE[Number(skill?.tier)] ?? 1)
    }
    payloads.push(row)
  }

  for (const match of text.matchAll(/apply\s+both\s+([A-Za-z][A-Za-z\s]+?)\s+and\s+([A-Za-z][A-Za-z\s]+?)(?:\s*\(([^)]*)\)|(?=\s+to\b)|[.;,]|$)/gi)) {
    addPhrase(match[1].trim(), match[3], { fromApply: true })
    addPhrase(match[2].trim(), match[3], { fromApply: true })
  }
  ...
  for (const match of text.matchAll(/(?:\d+%\s+chance to|chance to|may)\s+apply\s+([A-Za-z][A-Za-z\s]+?)(?:\s*\(([^)]*)\)|(?=\s+to\b)|[.;,]|$)/gi)) {
    ...
    if (/attackers?\s+(?:that|who)\s+hit/i.test(sentence)) continue
    addPhrase(match[1].trim(), match[2], { fromChance: true, applyTo: 'target' })
  }

  for (const match of text.matchAll(/(?:Apply|apply)\s+([A-Za-z][A-Za-z\s]+?)(?:\s*\(([^)]*)\)|(?=\s+to\b)|[.;,]|$)/gi)) {
    if (/apply\s+both/i.test(match[0])) continue
    addPhrase(match[1].trim(), match[2], { fromApply: true })
  }

  for (const match of text.matchAll(/\b(?:applies?)\s+([A-Za-z][A-Za-z\s]+?)(?:\s*[—–-]|\s*\(([^)]*)\)|(?=\s+to\b)|[.;,]|$)/gi)) {
    addPhrase(match[1].trim(), match[2], { applyTo: 'target' })
  }

  for (const match of text.matchAll(/\band\s+(Burn|Bleeding|Fear|Immobilized|Exhausted|Poison|Weakened|Incapacitated|Knockdown)\b(?:\s*\(([^)]*)\))?/gi)) {
    addPhrase(match[1].trim(), match[2], { applyTo: 'target' })
  }

  for (const match of text.matchAll(/\bgain\s+([A-Za-z][A-Za-z\s]+?)\s+for\s+(\d+)\s*rounds?/gi)) {
    addPhrase(match[1].trim(), match[0], { applyTo: 'target' })
  }

  for (const match of text.matchAll(/\bare\s+(Immobilized|Exhausted|Weakened|Feared|Burning|Poisoned)\b(?:\s+for\s+(\d+)\s*rounds?)?/gi)) {
    addPhrase(match[1].trim(), match[0], { applyTo: 'target' })
  }

  for (const match of text.matchAll(/\bYou are\s+([A-Za-z][A-Za-z\s]+?)\s+for\s+(\d+)\s*rounds?/gi)) {
    addPhrase(match[1].trim(), match[0], { applyTo: 'self' })
  }
  ...
  for (const match of text.matchAll(/(\d+)%\s+([A-Za-z][A-Za-z\s]+?)(?:[.;,+]|\s+\+|$)/g)) {
    ...
    const sentence = text.slice(sentenceStart, match.index + match[0].length)
    if (/attackers?\s+(?:that|who)\s+hit/i.test(sentence)) continue
    addPhrase(match[2].trim(), text, { fromChance: true, applyTo: 'target' })
  }
