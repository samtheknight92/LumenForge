...
// ── 7. Premade roster shape (leveled generator — see build-premade-characters.mjs) ──
const premades = readJson('premade-characters')
const premadeDetails = []
const byCategory = { npc: 0, monster: 0, pedestrian: 0 }
for (const p of premades) {
  if (p.category in byCategory) byCategory[p.category] += 1
}
const starters = premades.filter(p => String(p.premadeId || '').startsWith('level5_')).length
// Level 5 starters are player quick-start sheets that happen to use category "npc" too.
const EXPECTED = { npc: 150 + starters, monster: 150, pedestrian: 45 }
for (const [category, expected] of Object.entries(EXPECTED)) {
  if (byCategory[category] !== expected) {
    premadeDetails.push({ level: 'warn', text: `Expected ${expected} ${category} premades, found ${byCategory[category]}` })
  }
}
if (starters !== 4) premadeDetails.push({ level: 'warn', text: `Expected 4 Quick-Start player sheets, found ${starters}` })
section('Premade character list', premadeDetails.length === 0, premadeDetails.length ? premadeDetails : [{ level: 'ok', text: `${premades.length} premade characters (${byCategory.npc} NPC, ${byCategory.monster} Monster, ${byCategory.pedestrian} Pedestrian, ${starters} Quick-Start sheets)` }])
